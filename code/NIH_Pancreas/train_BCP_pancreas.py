...
data_root, split_name = 'preprocess/data', 'pancreas'
...
u_weight = 1.5
connect_mode = 1
self_train_name = 'ema_self_train'
...
def pretrain(net1, optimizer, lab_loader_a, lab_loader_b, test_loader):
    ...
    logger.info("cutmix Pretain, patch_size: {}, save path: {}".format(96*patch_ratio, str(save_path)))
    ...
    for epoch in tqdm(range(1, pretraining_epochs + 1), ncols=70):
        ...
        for step, ((img_a, lab_a), (img_b, lab_b)) in enumerate(zip(lab_loader_a, lab_loader_b)):
            ...
            img_mask, loss_mask = generate_mask(img_a, patch_ratio)   
...
def ema_cutmix(net, ema_net, optimizer, lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b, test_loader):
    ...
    for epoch in tqdm(range(1, self_training_epochs+1)):
        ...
        for step, ((img_a, lab_a), (img_b, lab_b), (unimg_a, unlab_a), (unimg_b, unlab_b)) in enumerate(zip(lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b)):
            ...
            # """Mix input"""
            net3_input_unlab = unimg_a * img_mask + img_b * (1 - img_mask)
            net3_input_lab = img_a * img_mask + unimg_b * (1 - img_mask)

            """Supervised Loss"""
            mix_output_unlab = net(net3_input_unlab)[0]
            loss_1 = mix_loss(mix_output_unlab, uimg_a_plab.long(), lab_b, loss_mask, unlab=True)

            """Unsupervised Loss"""
            mix_output_lab = net(net3_input_lab)[0]
            loss_2 = mix_loss(mix_output_lab, lab_a, uimg_b_plab.long(), loss_mask)
...
def test_model(net, optimizer, test_loader):
    load_path = Path(result_dir) / self_train_name
    load_net(net, load_path / 'best_ema_20_self.pth')
    print('Successful Loaded')
    avg_metric = test_calculate_metric(net, test_loader.dataset, s_xy=16, s_z=4)
    test_dice = avg_metric[0]
    print('Conctrust to SimCVD--(16, 16, 4) test_metric is: {}'.format(avg_metric))
    return avg_metric
...
if __name__ == '__main__':
    try:
        ...
        """Pre-train Stage"""
        max_dice_a = pretrain(net, optimizer, lab_loader_a, lab_loader_b, test_loader)
        """Self-train Stage"""
        max_dice_ema, max_list = ema_cutmix(net, ema_net, optimizer, lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b, test_loader)
        """Test"""
        avg_metric = test_model(net, optimizer, test_loader)
